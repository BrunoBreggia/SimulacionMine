...
def test_model(muestras, neuronas, activacion, batch_size, epocas):
    ...
    d_in = dimX + dimZ
    # batch_size = 1024
    modelo = StandardModel(nLayers=capas,
                           in_dimension=d_in,
                           neurons=neuronas,
                           activationFunc=activacion)

    if tipo == "Mine":
        model_mi = Mine(modelo, cuda='cpu')
    elif tipo == "MineEMA":
        model_mi = MineEMA(modelo, cuda='cpu')
    else:
        raise TypeError(f"No existe el modelo {tipo}")

    # Se escoje optimizador de tipo ADAM
    # learning_rate = 1e-4
    optimizer_mi = opt.Adam(model_mi.parameters(),
                            lr=1e-4,
                            betas=(0.9, 0.99),
                            weight_decay=0.001)

    # Return estimations for diferent amounts of epochs
    output_train, output_test = model_mi.train_model(X_train,
                                                     Z_train,
                                                     optimizer=optimizer_mi,
            ...
            for activacion in Activacion:
                for frac_batch in Batch_frac:
                    ...
                    for realizacion in range(REA):
                        ...
                        mi_ray_ids.append(
                            test_model.remote(muestras, neuronas, activacion,
                                              int(frac_batch * muestras),
                                              epocas))
                        # print(flush=True)
                        # print(f"Informacion mutua real: {mi}", flush=True)

                    for raID in mi_ray_ids:
                        temp = ray.get(raID)
                        est_train, est_eval, mi = temp
